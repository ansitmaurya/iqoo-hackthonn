...
export const Header: React.FC = () => {
  ...
  return (
    <>
      {/* Non-intrusive API Fallback Notification Toast if Live is requested but Backend is offline */}
      {fallbackActive && fallbackMessage && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span>
              <strong>Fallback Active:</strong> {fallbackMessage} (Full dashboard interactivity preserved with prototype seeds).
            </span>
          </div>
      ...
      <header style={{
        ...
        {/* Left: Stream Engine Status & DATA SOURCE Switcher */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {/* Dual Data Source Toggle Switcher */}
          <div style={{
            display: 'flex',
            alignItems: 'center',
            background: '#f1f5f9',
            ...
            <button
              ...
              title="Demo Mode: Prototype synthetic datasets (100% offline & functional)"
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '5px',
                ...
                letterSpacing: '0.4px',
                cursor: 'pointer',
                background: dataSourceMode === 'DEMO' ? 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)' : 'transparent',
                color: dataSourceMode === 'DEMO' ? '#ffffff' : '#64748b',
                boxShadow: dataSourceMode === 'DEMO' ? '0 2px 6px rgba(79, 70, 229, 0.35)' : 'none',
            ...
            <button
              onClick={() => setDataSourceMode('LIVE')}
              title="Live Mode: Flask REST API connected to Supabase / PostgreSQL"
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '5px',
                ...
                letterSpacing: '0.4px',
                cursor: 'pointer',
                background: dataSourceMode === 'LIVE' ? (isApiConnected ? 'linear-gradient(135deg, #059669 0%, #047857 100%)' : 'linear-gradient(135deg, #d97706 0%, #b45309 100%)') : 'transparent',
                color: dataSourceMode === 'LIVE' ? '#ffffff' : '#64748b',
                boxShadow: dataSourceMode === 'LIVE' ? '0 2px 6px rgba(5, 150, 105, 0.35)' : 'none',
                transition: 'all 0.2s ease'
              }}
            >
              {apiLoading ? (
          ...
          {/* Active Data Mode Connection Badge */}
          <div style={{
            display: 'flex',
            alignItems: 'center',
            gap: '6px',
            background: dataSourceMode === 'DEMO' ? 'rgba(99, 102, 241, 0.08)' : (isApiConnected ? 'rgba(5, 150, 105, 0.08)' : 'rgba(245, 158, 11, 0.08)'),
            border: `1px solid ${dataSourceMode === 'DEMO' ? 'rgba(99, 102, 241, 0.3)' : (isApiConnected ? 'rgba(5, 150, 105, 0.3)' : 'rgba(245, 158, 11, 0.3)')}`,
            borderRadius: '8px',
            padding: '5px 10px',
            fontSize: '11px',
            fontFamily: 'JetBrains Mono'
          }}>
            <Database size={13} color={dataSourceMode === 'DEMO' ? '#4f46e5' : (isApiConnected ? '#059669' : '#d97706')} />
            <span style={{ color: dataSourceMode === 'DEMO' ? '#4f46e5' : (isApiConnected ? '#059669' : '#d97706'), fontWeight: 700 }}>
              {dataSourceMode === 'DEMO' ? 'SOURCE: PROTOTYPE JSON' : (isApiConnected ? 'SOURCE: POSTGRES LIVE' : 'SOURCE: DEMO FALLBACK')}
            </span>
          </div>
