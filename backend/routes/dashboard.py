...
def get_dashboard_metrics():
    try:
        ...
        flagged_count = db_session.query(func.count(Transaction.id)).filter(
        ...
        low_count = db_session.query(func.count(Transaction.id)).filter(Transaction.risk_score < 35).scalar() or 0
        medium_count = db_session.query(func.count(Transaction.id)).filter(
            (Transaction.risk_score >= 35) & (Transaction.risk_score < 60)
        ).scalar() or 0
        high_count = db_session.query(func.count(Transaction.id)).filter(
            (Transaction.risk_score >= 60) & (Transaction.risk_score < 80)
        ).scalar() or 0
        critical_count = db_session.query(func.count(Transaction.id)).filter(Transaction.risk_score >= 80).scalar() or 0

        open_alerts = high_count + critical_count
        crit_alerts = critical_count

        fraud_rate = round((flagged_count / total_count * 100), 2) if total_count > 0 else 0.0

        # Recent 10 transactions from Supabase
        ...
        return jsonify({"success": False, "error": f"Failed to compute dashboard metrics: {str(e)}"}), 500
